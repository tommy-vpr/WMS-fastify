...
export const productRepository = {
   ...
   */
  async findByShopifyId(shopifyProductId: string): Promise<Product | null> {
    return prisma.product.findUnique({
   ...
   * Find variant by UPC
   */
  async findVariantByUpc(upc: string): Promise<ProductVariant | null> {
    return prisma.productVariant.findUnique({
      where: { upc },
    });
  },
   ...
   * Update product
   */
  async update(id: string, data: Partial<UpsertProductData>): Promise<Product> {
